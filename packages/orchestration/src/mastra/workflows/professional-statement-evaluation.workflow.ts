...
const rawCheckpointSchema = z.object({
	score: z.union([z.literal(0), z.literal(0.25), z.literal(0.5), z.literal(0.75), z.literal(1)]),
...
const fetchEvaluationContextStep = createStep({
	...
	execute: async ({ inputData, requestContext }) => {
		...
		const jobPreferences =
			typeof profile.jobPreferences === 'object' && profile.jobPreferences !== null
				? (profile.jobPreferences as Record<string, unknown>)
				: {};
		const hasAuthenticityContext = Boolean(
			narrative.trim() || narrativeSummary || Object.keys(jobPreferences).length,
		);
...
function statusForScore(score: number): ProfessionalStatementCheckpointEvaluation['status'] {
	if (score >= 0.75) {
		return 'met';
	}
	if (score > 0) {
...
const evaluateStatementStep = createStep({
	...
	description: 'Evaluates a statement against the six professional checkpoints',
	...
	execute: async ({ inputData }) => {
		...
		const checkpoints = Object.fromEntries(
			Object.entries(response.object.checkpoints).map(([key, checkpoint]) => [
				key,
				{
					...checkpoint,
					status:
						key === 'authenticity' && !inputData.hasAuthenticityContext
							? 'insufficient-context'
							: statusForScore(checkpoint.score),
				},
			]),
		) as z.infer<typeof professionalStatementEvaluationSchema>['checkpoints'];
...
export const professionalStatementEvaluationWorkflow = createWorkflow({
	...
	description: 'Evaluates a professional statement against six evidence-based checkpoints',
	requestContextSchema: z.object({
		[MASTRA_AUTH_TOKEN_KEY]: z.string(),
	}),
	inputSchema,
