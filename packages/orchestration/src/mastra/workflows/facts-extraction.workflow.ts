...
const step = createStep(factsExtractorAgent, {
	structuredOutput: {
		schema: z.object({
			jobFacts: z.array(z.string()),
...
const extractionStep = createStep({
	id: 'extract-facts',
	description: 'Extracts facts from career narrative',
	inputSchema: z.object({
		careerNarrative: z.string(),
	}),
	outputSchema: z.any(),
	execute: async ({}) => {
		const agent = factsExtractorAgent;
...
const factsExtractionWorkflow = createWorkflow({
...
factsExtractionWorkflow.commit();

export { factsExtractionWorkflow };
