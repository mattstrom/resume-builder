...
export const HighlightRegion: FC<HighlightRegionProps> = observer(({ path, label, children }) => {
	...
	const isConceptEvidence = inspectStore.isConceptEvidenceHighlighted(path);
	...
	return (
		<ContextMenu>
			<ContextMenuTrigger asChild>{React.cloneElement(child, childProps)}</ContextMenuTrigger>
			<ContextMenuContent>
				<ContextMenuItem onSelect={() => inspectStore.toggleSelected(path, label)}>
					{isInChat ? (
						<>
							<MessageSquareX />
							Remove from chat
