...
export const AppShell: FC<AppShellProps> = observer(({ children, primaryNav, toolbar }) => {
	const sidebarPanelRef = useRef<PanelImperativeHandle>(null);
	const { uiStateStore } = useStore();
	const { chatOpen, sidebarOpen } = uiStateStore;

	const sidebarDefaultSize = Number(localStorage.getItem('sidebar-panel-size')) || 15;
	const chatDefaultSize = Number(localStorage.getItem('chat-panel-size')) || 25;
		...
		>
			...
			<PanelGroup orientation="horizontal" className="flex-1 min-h-0">
				<Panel
					...
					minSize="8%"
					maxSize="30%"
					onResize={(size) => localStorage.setItem('sidebar-panel-size', String(size))}
				>
					<AppSidebar />
				</Panel>
				<PanelResizeHandle className="editor-resize-handle" />
				<Panel id="main">
					<SidebarInset className="h-full">{children ?? <Outlet />}</SidebarInset>
				</Panel>
				{chatOpen && (
					<>
						<PanelResizeHandle className="editor-resize-handle" />
						<Panel
							...
							maxSize="40%"
							onResize={(size) =>
								localStorage.setItem('chat-panel-size', String(size))
							}
						>
							<ChatPanel />
						</Panel>
