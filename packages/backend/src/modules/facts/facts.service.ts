...
export class FactsService {
  ...
  async update(uid: string, id: string, dto: UpdateFactDto): Promise<FactWithoutEmbedding> {
    ...
    return this.prisma.fact.update({ where: { id }, data: dto });
  }
  ...
  async setEmbedding(uid: string, id: string, vector: number[]): Promise<void> {
    ...
    await this.prisma.$executeRawUnsafe(
      `UPDATE "${SCHEMA}"."Fact" SET embedding = $1::vector WHERE id = $2`,
      formatted,
      id,
    );
  }
  ...
  async createExpression(uid: string, factId: string, dto: CreateExpressionDto): Promise<Expression> {
  ...
  async deleteExpression(uid: string, factId: string, id: string): Promise<void> {
    ...
    if (!expr) throw new NotFoundException(`Expression ${id} not found`);
  ...
  async unlinkFact(resumeId: string, factId: string): Promise<void> {
    ...
    if (!link) throw new NotFoundException(`ResumeFact not found`);
