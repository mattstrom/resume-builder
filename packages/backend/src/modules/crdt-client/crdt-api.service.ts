...
export class CrdtApiService {
	...
	async readDocument(documentName: string): Promise<{
		nodes: NarrativeNode[];
		professionalStatements: Array<{
			id?: string;
			label?: string;
			text?: string;
		}>;
	...
	async applyResumePatch(
		...
		if (!res.ok) {
			const body = await res.text();
			this.logger.error(
				`CRDT API resume patch failed: ${res.status} ${body}`,
			);
			throw new Error(`CRDT API error: ${res.status}`);
		}
	...
	async replaceResumeXml(
		...
		if (!res.ok) {
			const body = await res.text();
			this.logger.error(
				`CRDT API replace-xml failed: ${res.status} ${body}`,
			);
			throw new Error(`CRDT API error: ${res.status}`);
		}
		return res.json() as Promise<{
